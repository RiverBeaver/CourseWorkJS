...
function choosePage() {
  if (!hash) {
    ...
    showError();
    body.querySelector('.errorOverlay .text').innerText =
...
function gotoRegistrationForm() {
  document
    ...
    .append(view.get('registrationFunctional'));
  registration();
}
...
function showProfileOptions() {
  ...
  body.onclick = function (click) {
    if (
      !(
        click.target.closest('.profileOptions') || click.target.closest('.user')
      )
    ) {
      hideProfileOptions();
      body.onclick = '';
...
export function hideProfileOptions() {
  if (document.querySelector('.profileOptions'))
    body.removeChild(document.querySelector('.profileOptions'));
}
