  ...
  allFeeds.forEach((elem) =>
    elem.addEventListener('mousedown', (e) => {
      if (e.target.closest('.taskForm')) {
        ...
        let mousePositionX =
          startX - (startX - elem.offsetLeft - e.offsetX - e.target.offsetLeft);
        let mousePositionY =
          startY - (startY - elem.offsetTop - e.offsetY - e.target.offsetTop);
        ...
        document.onmouseup = () => {
          ...
          if (currentBoard && currentBoard !== elem) {
            edit({
              id: taskElem.id,
              currentBoard: currentBoard.querySelector('.tasks').id,
            });
            taskBoard.innerHTML = '';
...
function taskPageFunctional(task) {
  ...
  document
    .querySelector('.task')
    .addEventListener('mousedown', () => (mouseDowtMain = false));

  document.onclick = function (e) {
    if (
      !e.target.closest('.task') &&
      !e.target.closest('.taskForm') &&
...
export function edit(task) {
  if (currentTaskOpen && task) {
    ...
    const description = currentTaskOpen.querySelector(
      '.description textarea'
    ).value;
    const priority = currentTaskOpen.querySelector(
      '.chooseImportance input:checked'
    ).value;
    const status = currentTaskOpen.querySelector('.chooseBoard select').value;
    const isPrivate = !!+currentTaskOpen.querySelector(
      '.chooseAccess input:checked'
    ).value;
  ...
  if (task && task.lastUserName && task.newUserName && task.avatar) {
    mainDB.editWhenChangUser(task.lastUserName, task.newUserName, task.avatar);
    mainDB.saveInLocalStorage();
