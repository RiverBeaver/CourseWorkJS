...
export function taskCreate(e) {
  ...
  const resetTaskButton = document.querySelector(
    '.taskCreationLayout button.reset'
  );
  const createTaskButton = document.querySelector(
    '.taskCreationLayout button.create'
  );
...
function TaskBoardSelected(elem, select) {
  ...
  for (let i = 0; i < select.length; i++) {
    ...
    if (select[i].innerText === boardName)
...
function create() {
  ...
  const isPrivate = !!+document.querySelector('.chooseAccess input:checked')
    .value;
