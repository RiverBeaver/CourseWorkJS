...
export function showTaskFeed(
  completeBoardFilter = { status: 'complete' },
  inProgressBoardFilter = { status: 'in-progress' },
  toDoBoardFilter = { status: 'to-do' }
) {
  mainDB.getFromLocalStorage();

  taskFeedView.getFeed(0, 20, completeBoardFilter, 'Complete');
  taskFeedView.getFeed(0, 20, inProgressBoardFilter, 'In progress');
  taskFeedView.getFeed(0, 20, toDoBoardFilter, 'To do');
