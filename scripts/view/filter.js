...
export class FilterView {
  ...
  display() {
    ...
    if (this._filterObj) {
      const filterLayoutInputs = document.querySelectorAll(
        '.filterLayout input'
      );
      ...
      for (const input of filterLayoutInputs) {
        if (
          this._filterObj.priority &&
          this._filterObj.priority === input.value
        )
          input.checked = true;
        if (
          this._filterObj.isPrivate !== undefined &&
          this._filterObj.isPrivate === input.value
        )
          input.checked = true;
      }
    }
